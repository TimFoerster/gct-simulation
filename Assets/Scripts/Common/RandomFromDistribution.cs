...
public static class RandomFromDistribution  {
	...
	/// </param>
	public static float RandomRangeNormalDistribution(RandomNumberGenerator rng, float min, float max,
	                                                  ConfidenceLevel_e confidence_level_cutoff = 0 /*, float confidence_level_cutoff*/) {
		...
		do {
			random_normal_num = RandomNormalDistribution(rng, mean, sigma);
	...
	public static float RandomNormalDistribution(RandomNumberGenerator rng, float mean, float std_dev) {
		
		// Get random normal from Standard Normal Distribution
		float random_normal_num = RandomFromStandardNormalDistribution(rng);
	...
	/// </returns>
	public static float RandomFromStandardNormalDistribution(RandomNumberGenerator rng) {
		...
		float seed = rng.NextInt(0,2) == 0 ? u : v;
	...
	public static float RandomRangeSlope(RandomNumberGenerator rng, float min, float max, float skew, Direction_e direction) {
		return  min + RandomFromSlopedDistribution(rng, skew, direction) * (max-min);
	}
	...
	/// <param name="skew">The difference in height between max and min of curve.</param>
	public static float RandomFromSlopedDistribution(RandomNumberGenerator rng, float skew, Direction_e direction) {
		...
		float u = rng.Range(0.0f, max_cdf);
	...
	public static float RandomRangeLinear(RandomNumberGenerator rng, float min, float max, float slope) {

		float val = RandomLinear(rng, slope);
	...
	public static float RandomLinear(RandomNumberGenerator rng, float slope) {

		float abs_value = RandomFromLinearWithPositiveSlope(rng, Mathf.Abs(slope));
		if (slope < 0) {
			return 1 - abs_value;
		}
		else {
	...
	// Returns random in range [0,1] with linear distribution of given slope.
	private static float RandomFromLinearWithPositiveSlope(RandomNumberGenerator rng, float slope) {

		if (slope == 0) { 
			return rng.Range(0.0f, 1.0f);
		}

		float x, y;
		do {
			x = rng.Range(0.0f, 1.0f);
			y = rng.Range(0.0f, 1.0f);
			if (slope < 1) {
				y -= (1 - slope) / 2.0f;
			}
		} while (y > x * slope);
	...
	public static float RandomRangeExponential(RandomNumberGenerator rng, float min, float max, float exponent, Direction_e direction) {
		return min + RandomFromExponentialDistribution(rng, exponent, direction) * (max-min);
	}
	...
	/// </param>
	/// <param name="direction">The direction for the curve (right/left).</param>
	public static float RandomFromExponentialDistribution(RandomNumberGenerator rng, float exponent, Direction_e direction) {
	...
	/// </param>
	public static int RandomChoiceFollowingDistribution(RandomNumberGenerator rng, List<float> probabilities) {
		...
		// Choose from CDF:
		float cdf_value = rng.Range(0.0f, cdf[probabilities.Count-1]);
